...
int show_uci_option(struct ubus_context *ctx, struct ubus_object *obj,
	...
	blob_buf_init(&blob, 0);
	blobmsg_add_string(&blob, "Description", "Output /etc/config/ubus-sample");
...
int update_uci_option(struct ubus_context *ctx, struct ubus_object *obj,
	...
	snprintf(update_data, sizeof(update_data), "ubus-sample.test.%s=%s", option, value);
	...
	blob_buf_init(&blob, 0);
	blobmsg_add_string(&blob, "Result", "[UCI SET SUCCESS] Please check ubus-sample config!!");
	blobmsg_add_string(&blob, "Note", "ex) user~# uci show ubus-sample");
	ubus_send_reply(ctx, req, blob.head);
